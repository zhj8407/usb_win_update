...
/// Our USB class id that driver uses to register our device.
#define PLCM_VSC_USB_CLASS_ID \
{0x6b11e5bb, 0xf5ca, 0x4902, {0x96, 0x15, 0x77, 0x3b, 0x3d, 0x2f, 0x2f, 0xec}};
...
/** \brief Synchronously sends data to the default endpoint.

  @param[in] adb_interface A handle to interface object created with
         AdbCreateInterface call.
  @param[in] setup_packet The 8-byte setup packet of type WINUSB_SETUP_PACKET
  @param[in] buffer Pointer to the buffer containing the data to be send. Can be NULL
  @param[in] bytes_to_send Number of buffer bytes to be send.
  @param[out] bytes_transferred Number of buffer bytes transferred. Can be NULL.
  @return true on success and false on failure. If false is
          returned GetLastError() provides extended error information.
*/
ADBWIN_API bool __cdecl AdbDefaultEndpointReadWriteSync(ADBAPIHANDLE adb_interface,
                                     bool is_in,
                                     void *setup_packet,
                                     void *buffer,
